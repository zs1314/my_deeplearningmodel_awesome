...
from student_mobilenet_model import MobileNetV2
...
# 处理数据
train_dataset = ImageFolder(root='./grapes images/train', transform=train_data_transforms)
test_dataset = ImageFolder(root='./grapes images/validation', transform=test_data_transforms)
...
# # 创建网络模型
# stu_model = MobileNetV2(num_classes=4)  # 迁移学习搞的是别人训练好的参数，但是还是要自己创建模型(model中创建的)，并建立，但是然后在这里直接加载别人的参数
#
# # 加载训练好的参数
# model_weight_path = 'stu_mobileNet_method_pre.pth'  # Pytorch训练好的权重参数（不是整个模型）
# pre_weight = torch.load(model_weight_path)  # 加载权重
# stu_model.load_state_dict(pre_weight, strict=False)

# 创建网络模型
num_class=4
stu_model=MobileNetV2(num_class)
stu_model=stu_model.to(device)  # 将训练设备放到cuda上

# 由于最后输出的类别有1000个，所以这里改变输出类别，添加一个全连接层
inchannel = stu_model.classifier[1].in_features
# print(model.classifier[1])
stu_model.classifier[1] = nn.Linear(inchannel, 4)
stu_model = stu_model.to(device)

# 建立已经训练好的教师模型
teacher_ResNet = resnet34(num_classes=4)
# 加载权重数据
model_weight_path = './ResNet_method.pth'
teacher_ResNet.load_state_dict(torch.load(model_weight_path))
teacher_ResNet = teacher_ResNet.to('cuda')

# 损失函数，这里的多分类问题仍采用交叉熵损失函数
# hardLoss
hard_loss = CrossEntropyLoss(weight=None)
...
# 定义tensorboard
writer = SummaryWriter(log_dir="stu_mobileNet_trend")

# 定义优化器
learning_rate = 0.0002
# optimizer=torch.optim.SGD(params=model.parameters(),lr=learning_rate)
# optimizer=torch.optim.Adam(params=model.parameters(),lr=learning_rate)
optimizer = torch.optim.Adam(stu_model.parameters(), lr=learning_rate)
...
for i in range(epoch):
    ...
    for data in train_loader:
        ...
        # 教师模型预测
        with torch.no_grad():
            teacher_preds = teacher_ResNet(imgs)
        teacher_ResNet.eval()
        stu_model.train()  # 这是必须的，由于有Dropout层，为了保护、固定参数
        # 学生模型预测
        student_preds = stu_model(imgs)
        # 计算hard_loss
        student_loss = hard_loss(student_preds, targets)  # 计算误差（损失），方便下面反向传播，计算梯度

        # 计算蒸馏后的预测结果以及soft_loss
        ditillation_loss=soft_loss(
            torch.log_softmax(student_preds/temp,dim=1),
            torch.softmax(teacher_preds/temp,dim=1)
        )

        # 将hard_loss与soft_loss加权求和
        loss=alpha*student_loss+(1-alpha)*temp*temp*ditillation_loss
        ...
        # 打印数据
        if train_step % 50 == 0:
            print("训练次数：%d   loss_single：%f" % (train_step, loss))
        # 本轮训练结束
    ...
    with torch.no_grad():  # 关闭梯度，测试时是不能更新参数的，所以也就不需要梯度，为避免梯度在测试中被污染，直接关闭
        ...
        for test_data in test_loader:
            ...
            output_test = stu_model(test_imgs)
            loss_test_single = hard_loss(output_test, test_targets)
    ...
    if average_accuracy > best_acc:
        ...
        torch.save(stu_model.state_dict(), "stu_mobileNet_method.pth")  # 保留模型的超参数
