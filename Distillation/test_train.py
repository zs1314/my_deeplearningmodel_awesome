...
from my_swish_mobilenet_model import CustomMobileNetV2
...
# 创建网络模型
num_class=4
model = CustomMobileNetV2(num_class)  # 迁移学习搞的是别人训练好的参数，但是还是要自己创建模型(model中创建的)，并建立，但是然后在这里直接加载别人的参数

# 加载训练好的参数
model_weight_path = 'test_mob_swish_pre.pth'  # Pytorch训练好的权重参数（不是整个模型）
...
# 定义tensorboard
writer = SummaryWriter(log_dir="test_swish")
...
for i in range(epoch):
    ...
    print("经过第{}轮训练   Loss：{}".format(i + 1, total_test_loss/len(test_dataset)))
    print("经过第{}轮训练   准确率：{}".format(i + 1, average_accuracy))

    # add_scalar绘制图像
    writer.add_scalar(tag="Loss", scalar_value=(total_test_loss/len(test_dataset)), global_step=i + 1)
    ...
    if average_accuracy > best_acc:
        ...
        torch.save(model.state_dict(), "test_mob_swish.pth")  # 保留模型的超参数
