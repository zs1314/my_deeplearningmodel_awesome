...
test_loader=DataLoader(dataset=test_dataset,batch_size=36,shuffle=False,num_workers=0)
...
# 创建网络模型
num_class=5
model=GoogLeNet(num_classes=num_class,aux_logits=True,init_weights=True)
...
# 定义tensorboard
writer=SummaryWriter(log_dir="GoogLeNet_trend")
...
for i in range(epoch):
    ...
    for data in train_loader:
        ...
        output,aux_logits1,aux_logits2=model(imgs)
        loss_single1=Loss(output,targets)  # 计算误差（损失），方便下面反向传播，计算梯度
        loss_single2 = Loss(aux_logits1, targets)
        loss_single3 = Loss(aux_logits2, targets)
        loss_single=loss_single1+loss_single2*0.3+loss_single3*0.3  # 这里的权重占比根据论文得来的
    ...
    model.eval()  # self.train为False,不进行辅助分类器，本来辅助分类只是帮我们更好的更新、优化参数，在一轮中验证时，就不需要了，只返回一个输出
    ...
    if average_accuracy>best_acc:
        ...
        torch.save(model.state_dict(),"GoogLeNet_method.pth")  # 保留模型的超参数
